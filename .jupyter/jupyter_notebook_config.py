...
## ユーザーがノートブックを root として実行できるようにするかどうか。
#  Default: False
...
## `set_secure_cookie` に渡す追加のキーワード引数。詳細は tornado の set_secure_cookie のドキュメントを参照。
#  Default: {}
...
## 非推奨: 圧縮された JS ファイルを使用するかどうか。主に開発中に JS が再コンパイルされるのを回避するために使用します。
#  Default: False
# c.NotebookApp.ignore_minified_js = False

## (bytes/sec) ストリーム出力が送信制限される前に iopub で送信可能な最大レート。
#  Default: 1000000
...
## ノートブックサーバ拡張としてロードする Python モジュールの辞書。エントリー値を使用して拡張のロードを有効または無効にすることができます。
#  拡張子はアルファベット順にロードされます。
#  Default: {}
...
## 非推奨 nbserver_extensions 辞書を代わりに使用して下さい
#  Default: []
...
## tornado HTTPServer の SSL オプションを指定します。 詳しくは tornado のドキュメントを参照。
#  Default: {}
...
## Jupyterノートブックが使用する tornado.web.Application のオーバーライドを指定します。
#  Default: {}
...
#  
#  この値は :meth:`WebSocketHandler.get_compression_options` から返されます。 None (default)
#  の場合は圧縮は無効になります。 辞書 (空でも良い) の場合は圧縮が有効になります。
#  
...
#  Choices: any of ['sha512', 'sha3_224', 'sha3_256', 'sha1', 'sha224', 'sha3_384', 'sha3_512', 'sha256', 'sha384', 'blake2b', 'blake2s', 'md5']
#  Default: 'sha256'
